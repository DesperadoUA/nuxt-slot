<template>
	<div class="casino_item">
		<div class="casino_item_thumbnail">
			<img :src="value.thumbnail" loading="lazy" width="200" height="100" @click="refActivate" />
			<img :src="value.marker" class="casino_item_marker" width="38" height="38" />
			<div class="casino_item_licensed" v-if="value.licensed.length !== 0">
				<img v-for="licensed in value.licensed" :src="licensed" :key="licensed" loading="lazy" width="45" height="45" />
			</div>
		</div>
		<div class="casino_item_rating casino_item_column ">
			<div>
				<div class="casino_item_title">{{ value.title }}</div>
				<div class="casino_item_subtitle">{{ value.sub_title }}</div>
				<div class="casino_item_rating_wrapper">
					<div class="casino_item_rating_container">
						...
						<div>
							<span class="casino_item_ratin_value">{{ value.rating | rating }}</span
							>/10
						</div>
					</div>
				</div>
		...
		<div class="casino_item_packet casino_item_column ">
			<div v-if="value.bonuses.length != 0" class="casino_item_packet_item">
				<div class="casino_item_title_packet">{{ value.bonuses[0].bonuses_title }}</div>
				<div class="casino_item_sub_title_packet">{{ value.bonuses[0].bonuses_value }}</div>
				<div class="casino_item_title_packet">{{ value.bonuses[0].bonuses_sub_title }}</div>
			</div>
		</div>
		<div class="casino_item_weiger casino_item_column ">
			<div v-if="value.bonuses.length >= 2" class="casino_item_packet_item">
				<div class="casino_item_title_packet">{{ value.bonuses[1].bonuses_title }}</div>
				<div class="casino_item_sub_title_packet">{{ value.bonuses[1].bonuses_value }}</div>
				<div class="casino_item_title_packet">{{ value.bonuses[1].bonuses_sub_title }}</div>
			</div>
		</div>
		<div class="casino_item_buttons casino_item_column ">
			<div class="casino_item_buttons_box">
				...
				<NuxtLink class="btn_review" no-prefetch :to="value.permalink">{{ translates.REVIEW[config.LANG] }}</NuxtLink>
			</div>
		</div>
	</div>
</template>
...
import refAction from '~/mixins/refAction'
export default {
	name: 'app_casino_loop',
	props: ['posts'],
	mixins: [translateMixin, refAction],
	filters: {
		rating(item) {
