<template>
	<amp-script layout="container" :src="`${config.BASE_URL[config.LANG]}/amp-blog.js`" class="sample">
				...
				<div class="blog_card_item" v-for="item in currentPosts" :key="item.id">
					<div class="blog_card_item_left">
						<div class="blog_card_item_img_wrapper">
							<amp-img :src="item.thumbnail" class="blog_card_item_thumbnail" width="320" height="190"></amp-img>
						</div>
					</div>
					<div class="blog_card_item_right">
						...
						<div class="blog_card_item_author">
							<p>
								{{ translates.REVIEW_AUTHOR[config.LANG] }}: <span>{{ item.author }}</span> {{ item.date | post_data }}
							</p>
						</div>
						...
						<div class="blog_card_item_btn_wrapper">
							<NuxtLink no-prefetch :to="`${config.AMP_PREFIX}${item.permalink}`" class="blog_card_item_btn">
