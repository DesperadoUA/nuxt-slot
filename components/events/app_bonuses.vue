		...
		<div class="container">
			...
			<div class="bonus_item" v-for="(item, i) in value.event" :key="i" itemscope itemtype="http://schema.org/Event">
				<div class="bonus_item_thumbnail">
					<img itemprop="image" :src="item.thumbnail" loading="lazy" width="200" height="100" />
				</div>
				<div class="bonus_item_action">
					<div class="bonus_item_ttl" v-html="item.title" itemprop="name"></div>
					<div class="bonus_item_time">
						с <span itemprop="startDate" :datetime="item.start">{{ item.start }}</span>
						{{ translates.BEFORE[config.LANG] }}
						<span itemprop="endDate" :datetime="item.end">{{ item.end }}</span>
					</div>
					<div class="bonus_item_button_wrapper">
						<button class="bonus_item_button" @click="refActivate">{{ translates.GET[config.LANG] }}</button>
						<button class="bonus_item_button" @click="activate(item)">
