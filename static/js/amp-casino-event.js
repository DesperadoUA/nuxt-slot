{
	const buttons = document.querySelectorAll('.jsReferralPopUp')
	const modalContainer = document.querySelector('.referralModalContainer')
	const dataSetRelativeCasino = modalContainer.attributes.filter(item => item.name === 'data-relative-casino')
	const relativeCasino = dataSetRelativeCasino.length ? JSON.parse(dataSetRelativeCasino[0].value) : []
	const dataSetTitle = modalContainer.attributes.filter(item => item.name === 'data-title')
	const title = dataSetTitle.length ? dataSetTitle[0].value : ''
	const dataSetDescription = modalContainer.attributes.filter(item => item.name === 'data-description')
	const description = dataSetDescription.length ? dataSetDescription[0].value : ''
	const dataSetBtn = modalContainer.attributes.filter(item => item.name === 'data-translate-btn')
	const ReferralModalBtn = dataSetBtn.length ? dataSetBtn[0].value : ''
	const dataSetGoTo = modalContainer.attributes.filter(item => item.name === 'data-translate-go-to')
	const goTo = dataSetGoTo.length ? dataSetGoTo[0].value : ''
	const dataSetAmpPrefix = modalContainer.attributes.filter(item => item.name === 'data-ampprefix')
