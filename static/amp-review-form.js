{
    ...
    const dataSetInvalidPostalAddress = btn.attributes.filter((item) => item.name === 'data-translate-invalid-postal-address')
    ...
    const dataSetReviewAddedForModeration = btn.attributes.filter((item) => item.name === 'data-translate-review-added-for-moderation')
    const reviewAddedForModeration = dataSetReviewAddedForModeration.length ? dataSetReviewAddedForModeration[0].value : ""
